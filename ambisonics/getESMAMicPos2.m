function [azel,radius,sphType,maxOrder] = getESMAMicPos2(rotationNum,maxOrder,clockwiseFlag)
if nargin<3
    clockwiseFlag = 1;
end
if clockwiseFlag
...
azimu = azimu(:);
eleva = eleva(:);
azimu = azimu([1:3,5:16])+(0:dAzi:dAzi*(rotationNum-1));
azimu = mod(azimu(:),360);
eleva = repmat(eleva([1:3,5:16]),rotationNum,1);
