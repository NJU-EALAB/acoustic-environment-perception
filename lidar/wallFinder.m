...
[Angule,Distance,Quality] = importfile(lindarFileName);
a=[Angule,Distance,Quality];
a=sortrows(a,1);
Angule=a(:,1);
Distance=a(:,2);
Quality=a(:,3);
for i=1:length(Angule)-1
    if abs(Angule(i,:)-Angule(i+1,:))>0.1
        b=linspace(Angule(i,:), Angule(i+1,:), 50);
        c=linspace(Distance(i,:), Distance(i+1,:), 50);
        Angule=[Angule;b'];
        Distance=[ Distance;c'];
    end
end
...
%hough变换
[ H, theta, rho ] =hough(img,'RhoResolution',1,'Theta',-90:5:89);
% wallNum=4;
peaks=houghpeaks(H,wallNum,'threshold', ceil(0.01*max(H(:))));
lines = houghlines(img,theta,rho,peaks,'FillGap',60,'MinLength',1);
