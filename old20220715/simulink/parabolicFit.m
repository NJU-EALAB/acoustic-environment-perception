...
% copy from: phased.internal.parabolicFit
% revise for debug: Index exceeds the number of array elements
...
for ii = 1:M
    for jj = 1:N
        idx = k0(ii,jj);
        if (idx~=1)&&(idx~=M0)
            deltak(ii,jj) = 0.5*(x(idx-1,jj)-x(idx+1,jj))./(x(idx+1,jj)+x(idx-1,jj)-2*x(idx,jj));
        end
    end
end
