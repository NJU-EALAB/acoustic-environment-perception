function [leg,labelhandles,outH,outM] = legend21b(varargin)
...
elseif narg > 0 && ...
    ...
    if isempty(ha)
        ...
        if isa(ha,'matlab.graphics.chart.Chart')
            try                
                [varargout{1:nargout}] = legend21b(ha,args{:}); %#ok<NASGU>
            catch e
                throw(e)
            end
            return
...
if narg >= arg && ...
   ...
   all(ismember(char(lower(args{arg})),{'off','deletelegend',...
    ...
    switch char(lower(args{arg}))
        ...
        case 'hide'
            set(legend21b(ha), 'Visible', 'off');
        case 'boxon'
            set(legend21b(ha), 'Box', 'on');
        case 'boxoff'
            set(legend21b(ha), 'Box', 'off');
        otherwise
